...
int leastsq_kkt_pool(double *b, double const *a, double const* c, double const* d,
                int n1, int n2, int n3, int neq, int *max_iter) {
  // check if n1 > 0, n2 > 0, n3 >= 0
  if (n1 <= 0 || n2 <= 0 || n3 < 0 || n1 < n2) {
    fprintf(stderr, "Error: n1 and n2 must be positive integers and n3 >= 0, n1 >= n2.\n");
  ...
  // Allocate memory for the augmented matrix and right-hand side vector
  int size = n2 + n3;
  double *aug = (double *)poolmalloc(size * size * sizeof(double));
  double *ata = (double *)poolmalloc(n2 * n2 * sizeof(double));
  double *atb = (double *)poolmalloc(size * sizeof(double));
  double *rhs = (double *)poolmalloc(size * sizeof(double));

  // evaluation of constraints
  double *eval = (double *)poolmalloc(n3 * sizeof(double));

  // index for the active set
  int *ct_indx = (int *)poolmalloc(n3 * sizeof(int));

  // index array for the LU decomposition
  int *lu_indx = (int *)poolmalloc(size * sizeof(int));
  ...
  while (iter++ < *max_iter) {
    ...
    // solve the KKT system
    ludcmp_pool(aug, lu_indx, n2 + nactive);
    lubksb(rhs, aug, lu_indx, n2 + nactive);
